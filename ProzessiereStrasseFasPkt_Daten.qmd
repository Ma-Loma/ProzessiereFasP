...
## Vorbemerkung Dateipfade
Da dieses Projekt von verschiedenen Menschen auf verschiedenen Computersystemen verwendet wird, sind hier alle im Projekt verwendeten Pfade versammelt, sodass sie ggf. leicht angepasst werden können:

```{r pfade}
daten_ordner_pfad <-
  "N:/4all.i4/I4.9 - Umweltbezogene Krankheitslast/04 - Daten/Lärm/Mainz-Bingen/"
#  das hier ist vermutlich von Tim: "D:/Mainz-Bingen/"

ausgabepfad_strabst <-
  paste0(daten_ordner_pfad,                                "Fassadenpunkte_mit_allen_Abstaenden.gpkg")

eingabepfad_strassennetz <-
  paste0(daten_ordner_pfad,                               
  "Strassennetz_LK22.gpkg")

eingabepfad_fasp_2024 <-
  paste0(daten_ordner_pfad,
         "Fassadenpunkte.shp")

eingabepfad_fasp_2025 <- "../Fassadenpunkte_EBA.gpkg"
```
...
Die Datei `{r} eingabepfad_strassennetz` wurde im Dezember 2025 gesendet von Nathalie Paris, LfU. Die Datei wird hier in R untersucht. Die Datei sollte alle mit Straßenverkehr belegten Straßen aus dem für die Umgebungslärmkartierung verwendeten Straßenverkehrsmodell beinhalten. Sie besteht aus einem Geopackage mit folgenden Layern:

```{r vorhandene_layer_str}
st_layers(eingabepfad_strassennetz)
```
...
strNetz22 <- st_read(dsn=eingabepfad_strassennetz, layer="strassennetz_lk22") %>% 
  st_transform(25832)
...
Die Datei `{r} eingabepfad_fasp_2024` wurde schon im Dezember 2024 gesendet von Nathalie Paris, LfU. Sie besteht aus einer Shapedatei mit folgenden Layern:

```{r layer_fp2024}
st_layers(eingabepfad_fasp_2024)
```
...
```{r einlesen_fp2024}
#| echo: false
fassadenpunkte_strasse24<-st_read(dsn=eingabepfad_fasp_2024)%>% 
  st_transform(25832)
...
Die Straßen haben verschiedene Baulastträger bzw. Bezeichnungen:

```{r strassenart}
strNetz22<-
  strNetz22 %>% 
  mutate(str_art=str_sub(STRASSE,1,1))# das hier extrahiert einfach den ersten Buchstaben: also A, B, K oder L
strNetz22$str_art %>% 
  unique()
...
Es könnte sein, dass Fassadenpunkte nahe an nicht kartierten Straßen liegen, die faktisch deren Immissionspegel dominieren - somit würden die Pegel im vorliegenden Modell unterschätzt.
Um das untersuchen zu können, wird hier der Abstand der Fassadenpunkte zur nächsten Straße im Straßennetz berechnet.
Die Ergebnisse werden dann zu den Fassadenpunkten hinzugefügt und wieder als Shapedatei `{r} ausgabepfad_strabst` abgespeichert.


```{r ermittle_strassenabstand}
# recht Rechenaufwändiger Schritt (ca. 15 min)
# zum Probieren kann hier ein kleinerer Anteil der Fassadenpunkte verwendet werden
verwendeter_anteil<-1 # muss 1 sein, wenn alles verwendet wird; zum Spielen ginge z.B. 0.01

# Alte Dateien löschen für sauberen Neustart
if (file.exists(ausgabepfad_strabst)) {
  base_path <- tools::file_path_sans_ext(ausgabepfad_strabst)
  # Lösche alle Endungen (.shp, .shx, .dbf, .prj etc.)
  files_to_delete <- list.files(dirname(ausgabepfad_strabst), 
...
fassadenpunkte_erweitert <- fassadenpunkte_strasse24 %>% 
  slice_sample(prop=verwendeter_anteil) %>% 
...
st_write(fassadenpunkte_erweitert, dsn = ausgabepfad_strabst, delete_dsn = TRUE)
...
entf_stat<-hausstatistik %>%
  filter(lautester_leisester == "max") %>%
  mutate(abst_start = breaks[findInterval(mean_strabst, breaks)],
         abst_end =
           breaks[findInterval(mean_strabst, breaks) + 1]) %>%
  group_by(dominante_gattung,abst_start,abst_end) %>%
  summarise(Anzahl = n(),
            gemittelter_lautester_LDEN = mean(LDEN, na.rm = TRUE))

entf_stat%>%
  ungroup() %>% 
  group_by(dominante_gattung) %>% 
  arrange(abst_start,.by_group = TRUE) %>%
  ...
  pivot_longer(cols=c(gemittelter_lautester_LDEN, Anzahl,kumulativer_Anteil),
               ...
               values_to="wert") %>%
  ggplot() +
  geom_step(aes(x = abst_start, y = wert,color = dominante_gattung))+
...
hausstatistik %>%
  ...
  ggplot() +
  geom_point(
    aes(x = mean_strabst, y = LDEN, color = dominante_gattung),
    size = 1,
    alpha = 0.5,
    shape = 20
  ) +
...
Die Datei `{r} eingabepfad_fasp_2025` wurde im Dezember 2025 gesendet von Nathalie Paris, LfU. Sie besteht aus einer gpkg-Datei mit folgenden Layern:

```{r vorhandene_layer}
st_layers(here(eingabepfad_fasp_2025))
```
...
```{r einlesen_fpEBA}
#| echo: false
fassadenpunkte_EBA<-st_read(dsn=eingabepfad_fasp_2025)%>% 
  st_transform(25832)
...
# library(gpkg)
# library(RSQLite)
# g <- geopackage(eingabepfad_strassennetz, connect = TRUE)
# g
# gpkg_list_tables(g)
...
![Zusammenschau Schienenlärm- und Straßenlärmfassadenpunkte](externe_Bilder/FassadenpunkteSchieneuStraße.png)
